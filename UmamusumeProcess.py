        ...
        while self.isAlive:
            ...
            if self.isDoneTutorial and self.is초기화하기 == False:
                ...
                if self.is연동하기:
                    ...
                    count = 메뉴_단축(img, self.device, self.InstancePort)
                    if count:
                        updateTime = time.time()
                    ...
                    count = 카카오_로그인(img, self.device, self.InstancePort)
                    if count:
                        updateTime = time.time()
                    ...
                    count = 확인하고_계속하기2(img, self.device, self.InstancePort)
                    if count:
                        updateTime = time.time()
                    ...
                    count = 확인하고_계속하기3(img, self.device, self.InstancePort)
                    if count:
                        updateTime = time.time()
                    ...
                    count = 계속하기(img, self.device, self.InstancePort)
                    if count:
                        updateTime = time.time()
                    ...
                    count = 정보_확인_중(img, self.device, self.InstancePort)
                    if count:
                        updateTime = time.time()
                    ...
                    count = Google_계정으로_로그인(img, self.device, self.InstancePort)
                    if count:
                        updateTime = time.time()
                    ...
                    count = 인증되지_않는_로그인_방법_입니다(img, self.device, self.InstancePort)
                    if count:
                        updateTime = time.time()
                    ...
                    count = 카카오_로그인_연동에_실패하였습니다(img, self.device, self.InstancePort)
                    if count:
                        updateTime = time.time()
            ...
            # count = 크롬_실행(img, self.device, self.InstancePort)
            ...
            # count = 크롬_실행2(img, self.device, self.InstancePort)
            # 
            # if count and is초기화하기:
            #     updateTime = time.time()
            #     # print("크롬_실행 " + str(count) + "개")
            ...
            if self.isDoneTutorial and self.is초기화하기:
                count = 파이어폭스_실행(img, self.device, self.InstancePort)
                if count:
                    updateTime = time.time()
                ...
                count = 파이어폭스_연결된_서비스_관리(img, self.device, self.InstancePort)
                if count:
                    updateTime = time.time()
                    # print("파이어폭스_실행 " + str(count) + "개")
                ...
                count = 파이어폭스_문제_닫기(img, self.device, self.InstancePort)
                if count:
                    updateTime = time.time()
                ...
                count = 연결된_서비스_관리(img, self.device, self.InstancePort)
                if count:
                    updateTime = time.time()
                ...
                count = 우마무스메_서비스(img, self.device, self.InstancePort)
                if count:
                    updateTime = time.time()
                ...
                count = 모든_정보_삭제(img, self.device, self.InstancePort)
                if count:
                    updateTime = time.time()
                ...
                count = 이_서비스의_모든_정보를_삭제하시겠습니까(img, self.device, self.InstancePort, hwndMain)
                if count:
                    updateTime = time.time()
                ...
                count = 이_서비스의_모든_정보를_삭제하시겠습니까2(img, self.device, self.InstancePort, hwndMain)
                if count:
                    updateTime = time.time()
                    # print("이_서비스의_모든_정보를_삭제하시겠습니까 " + str(count) + "개")
                    self.log("이_서비스의_모든_정보를_삭제하시겠습니까 " + str(count) + "개")
                ...
                count = 모든_정보_삭제_빨간_박스(img, self.device, self.InstancePort)
                if count:
                    updateTime = time.time()
                ...
                count = 비밀번호(img, self.device, self.InstancePort) # 크롬
                if count:
                    updateTime = time.time()
                ...
                count = 회원님의_소중한_정보_보호를_위해(img, self.device, self.InstancePort)
                if count:
                    updateTime = time.time()
                ...
                count = 제안된_로그인(img, self.device, self.InstancePort)
                if count:
                    updateTime = time.time()
                ...
                count = 자동완성_Continue(img, self.device, self.InstancePort) # 크롬
                if count:
                    updateTime = time.time()
                ...
                count = 자동완성_계속(img, self.device, self.InstancePort) # 크롬
                if count:
                    updateTime = time.time()
                ...
                count = 비밀번호_확인(img, self.device, self.InstancePort)
                if count:
                    updateTime = time.time()
            ...
            if self.isDoneTutorial and self.is초기화하기:    
                ...
                if time.time() >= updateTime + 5:
                    count = 로딩(img, self.device, self.InstancePort) # 크롬
                    if count:
                        updateTime = time.time()
            ...
            count = 카카오메일_아이디_이메일_전화번호(img, self.device, self.InstancePort, hwndMain)
            if count:
                updateTime = time.time()
            ...
            count = 로그인(img, self.device, self.InstancePort)
            if count:
                updateTime = time.time()
