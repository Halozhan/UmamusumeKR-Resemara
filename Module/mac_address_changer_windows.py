"""
reference: https://www.thepythoncode.com/code/make-a-mac-address-changer-in-python
"""
...
def get_connected_adapters_mac_address():
    ...
    for potential_mac in subprocess.check_output("getmac").decode(encoding=ENCODING).splitlines():
...
def change_mac_address(adapter_transport_name, new_mac_address):
    ...
    for interface in re.findall(rf"{network_interface_reg_path}\\\d+", output):
        ...
        interface_content = subprocess.check_output(f"reg QUERY {interface.strip()}").decode(encoding=ENCODING)
        if adapter_transport_name in interface_content:
            ...
            changing_mac_output = subprocess.check_output(f"reg add {interface} /v NetworkAddress /d {new_mac_address} /f").decode(encoding=ENCODING)
